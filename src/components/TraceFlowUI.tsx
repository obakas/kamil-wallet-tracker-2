    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-gray-800 rounded-xl shadow-lg border border-gray-700">
            ...
            <div className="flex items-center mb-6">
                ...
                <div>
                    <h2 className="text-2xl font-bold text-gray-100">Fund Trace Tool</h2>
                    <p className="text-sm text-gray-400">Track cryptocurrency flows between wallets</p>
                </div>
            </div>
            ...
            <div className="space-y-4 mb-6">
                <div>
                    ...
                    <textarea
                        id="walletInput"
                        className="w-full p-3 border border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all bg-gray-700 text-gray-100"
                        rows={3}
                        placeholder="Enter wallet addresses (comma, space, or newline separated)"
                        value={walletInput}
                        onChange={(e) => setWalletInput(e.target.value)}
                    />
                    <p className="mt-1 text-xs text-gray-400">Supports multiple addresses in any format</p>
                </div>

                <div>
                    <label htmlFor="tokenInput" className="block text-sm font-medium text-gray-300 mb-1">
                        Filter by Token
                    </label>
            ...
            ${loading ? 'bg-blue-700 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500 shadow-md hover:shadow-lg'}`}
                onClick={handleTrace}
                disabled={loading}
            >
                {loading ? (
            ...
            {!loading && filteredFlows.length > 0 && (
                <div className="mt-8">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-semibold text-gray-100">Transaction Flow</h3>
                        <span className="text-sm bg-blue-900 text-blue-100 py-1 px-3 rounded-full">
